...
/// <summary>
/// A fully-managed, CPU-only text analysis engine. It needs no GPU, no model download
/// and no external service, so responses are effectively instant.
///
...
///  • <b>Entities</b> — regex extractors for structured entities (emails, URLs, money,
///    dates, …) plus a capitalised-phrase heuristic for people / organisations / places.
/// </summary>
public sealed partial class SummarizationService : ISummarizationService
{
    public string EngineName => "Ark Lexical Engine (CPU)";
    ...
    public SummaryResult Summarize(string text, int maxSentences = 3)
    {
        var sw = Stopwatch.StartNew();
        text = (text ?? "").Trim();
        maxSentences = Math.Clamp(maxSentences, 1, 10);

        var result = new SummaryResult { Summary = "", Intent = "statement" };
        if (text.Length == 0)
        {
        ...
        var allWords = WordToken().Matches(text).Select(m => m.Value).ToList();

        result.SentenceCount = sentences.Count;
        result.WordCount = allWords.Count;

        // --- Word frequencies (lower-cased, stopwords removed) --------------
        ...
        (result.Intent, result.IntentConfidence) = ClassifyIntent(text, sentences);
        ...
        List<string> sentences, Dictionary<string, int> freq, int maxFreq, int maxSentences)
    {
        ...
        {
            var words = WordToken().Matches(sentences[i]).Select(m => m.Value.ToLowerInvariant()).ToList();
    ...
    private static (string Intent, double Confidence) ClassifyIntent(string text, List<string> sentences)
    {
    ...
    // ---- Entities ---------------------------------------------------------
    private static IReadOnlyList<Entity> ExtractEntities(string text)
    {
    ...
    private static string ClassifyProperNoun(string phrase)
    {
        ...
        // Multi-word capitalised phrases are most often names of people or organisations.
        return phrase.Contains(' ') ? "name" : "name";
    }
    ...
    private static readonly HashSet<string> CalendarWords = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    // ---- Lexicons ---------------------------------------------------------
    private static readonly HashSet<string> SentenceStarters = new(StringComparer.OrdinalIgnoreCase)
    {
